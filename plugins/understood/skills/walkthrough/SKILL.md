...
2. Validate until clean, with `--fix` when the only complaint is drift.
3. Render, then serve, then start the waiter.
4. Read the rendered page at the width the reader will use, and click through every block.

The renderer owns the sidebar, the ids, the editor links, and the escaping. Do not hand-edit the output html: the next render overwrites it, and the spec is the thing that gets reviewed, corrected, and re-rendered.
